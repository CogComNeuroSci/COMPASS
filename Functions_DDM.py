...
#%% DDM functions IC
# functions for DDM
def Incorrelation_repetition(means = np.array([0,1.6,0.5,1,0.6]),stds = np.array([2,1,0.3,0.3,0.4]), 
...
#     invTestimations = np.empty(npp)
# =============================================================================
    ParEstimation = np.empty((npp,len(means)))
    for pp in range(npp):

        ####Part 2: Data simulation for this participant####
        # generate the responses for this participant
        print (True_Par[pp])
        responses = simulate_responses(True_Par[pp],DDM_id,nreps)
...
def generate_parameters(means = np.array([0,1.6,0.5,1,0.6]),stds = np.array([2,1,0.3,0.3,0.4]), 
    ...
    if multivariate:
       ...
       #  while np.any(parameters[:,0] <= 0):
        ...
        parameters = pd.DataFrame(np.zeros((npp, len(means)), 
                            dtype = np.float32))
