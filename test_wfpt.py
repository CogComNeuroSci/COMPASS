...
if parameter_recovery:
    ...
    r,Tru,Est,ACC,RT = Incorrelation_repetition(means = np.array([0,1.18,0.5,0.5]),stds = np.array([0.3,0.33,0.05,0.25]), 
                                param_bounds = param_bounds, 
                                npp = npp,method = "Nelder-Mead",
                                ntrials = npp, DDM_id = "ddm", rep=1, nreps = 50, ncpu = 6)
    end_time = datetime.now()
    print("npp = {}, ntrails = {}".format(npp,ntrials))
    print("Power estimation started at {}.".format(start_time))
    print("Power estimation ended at {}.".format(end_time))
...
if parameter_range:
    ...
    save = 0
    if save:
        figure.savefig('D:/horiz/IMPORTANT/0study_graduate/Pro_COMPASS/COMPASS_DDM/results/test3/ParRange_0.4z0.5t.png')
